...
while True:
	...
	if id.lower()=="admin" and pwd.lower()=="admin":
		try:
			...
			val2=[(5000,"To Kill a Mockingbird","Harper Lee",3),
		...
		while True:
			...
			if menu1==1:
				...
				sql_connector.commit()
				sql_cursor.execute("UPDATE Catalog SET Qty=Qty-1 where Book_ID="+menu5)
				sql_connector.commit()
				print("")
				...
				while True:
					...
					if confirm.lower()=="y":
						...
						print(("New ID has been created with User ID: "+str(aid)).center(130))
						print("")
						break
				...
				if menu_2==1:
					while True:
						...
						try:
							sql_cursor.execute("SELECT * FROM Catalog WHERE Book_ID="+str(menu3))
							record=sql_cursor.fetchall()
							print("Do you wan to Delete",record[0][1],"by",record[0][2])
							confirm=str(input("Do want to proceed (Y/N): "))
							if confirm.lower()=="y":
								while True:
									...
									if menu4<=record[0][3]:
										...
										print("Only",record[0][3],"Books are available".center(130))
								break
							elif confirm.lower()=="n":
								print("Request Canceled".center(130))
								break
					...
					while True:
						...
						try:
							sql_cursor.execute("SELECT * FROM Catalog WHERE Book_Name like '%"+menu3+"%'")
							record=sql_cursor.fetchall()
							print("Do you want to Delete",record[0][1],"by",record[0][2])
							confirm=str(input("Do want to proceed (Y/N): "))
							if confirm.lower()=="y":
								while True:
									menu4=int(input("Quantity to be Deleted: "))
					...
					while True:
						...
						try:
							sql_cursor.execute("SELECT * FROM Catalog WHERE Author_Name LIKE '%"+str(menu3)+"%'")
							record=sql_cursor.fetchall()
							print("Do you want to Delete",record[0][1],"by",record[0][2])
							confirm=str(input("Do want to proceed (Y/N): "))
							if confirm.lower()=="y":
